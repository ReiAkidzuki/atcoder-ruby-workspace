...
ロックはリポジトリ直下の `.atcoder-contest-lock` に保存されます。
このファイルは Git の追跡対象外で、存在するだけでロックとして扱われます。
内容が壊れていても安全側に停止し、自動では期限切れになりません。
別の端末では共有されないため、コンテストに使用する端末ごとに実行してください。
...
終了前後の自動判定によって `.atcoder-contest-lock` を作成または削除することはありません。
